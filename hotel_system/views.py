...
def HotelList(request):
    hotel1 = Hotel()
    hotel_list = hotel1.hotels
    hotel_list_output = "<ul>"
    i = 0
    for h in hotel_list:
        hotel_list_output += "<li>" + h[1][i] + "</li>"
        i += 1
    hotel_list_output += "</ul>"
...
def HotelInCity(request):
    hotel1 = Hotel()
    hotel_list = hotel1.list_hotels_in_city()
    hotel_list_output = "<ul>"
    i = 0
    for h in hotel_list:
        hotel_list_output += "<li>" + h[0][i] + "</li>" + "<li>" + str(h[1][i]) + "</li>"
        i += 1
    hotel_list_output += "/ul"
    return HttpResponse(hotel_list_output)
...
def ReservationList(request):
    ...
    for h in reservation_list:
        reservation_list_output += "<li>" + h[i] + "<li>"
        i += 1
